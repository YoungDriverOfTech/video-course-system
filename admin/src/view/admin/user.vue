...
export default {
  ...
  methods: {
    add() {
      let _this = this;
      _this.user = {};
      $("#add-user-modal-form").modal("show");
    },

    edit(user) {
      let _this = this;
      // using parameter user directly will afffect table displayed in page, thus make a deep copy for parameter user
    ...
    list(page) {
      ...
      Loading.show();
      _this.$ajax
        .post("http://127.0.0.1:9000/system/admin/user/list", {
          page: page,
    ...
    save() {
      ...
      if (1 != 1
        ...
        || !Validator.length(_this.user.name, "nick name", 1, 50)
      ...
      _this.$ajax
        .post(
          "http://127.0.0.1:9000/system/admin/user/save",
          _this.user
        )
        .then((response) => {
          Loading.hide();
          let resp = response.data;
          if (resp.success) {
            $("#add-user-modal-form").modal("hide");
            _this.list(1);
            Toast.success("saved");
          } else {
    ...
    del(id) {
      ...
      Confirm.show("Please confirm your delete operation!", function () {
        Loading.show();
        _this.$ajax
          .delete("http://127.0.0.1:9000/system/admin/user/delete/" + id)
          .then((response) => {
            Loading.hide();
            let resp = response.data;
            if (resp.success) {
